...
REPOS=("AdvancedCalculaterBot" "CloudflareWorkerBot" "TelegramSemanticSearch" "FourthBotName")
BOTS=("calculator-bot" "cloudflare-bot" "semantic-search-bot" "fourth-bot")
...
for repo in "${REPOS[@]}"; do
    if [ ! -d "$BOTS_DIR/$repo" ]; then
        ...
        git clone "https://github.com/NimaTalebzadeh/$repo.git" "$BOTS_DIR/$repo"
    fi
done
...
# Fourth Bot
TELEGRAM_BOTTOKEN_FOURTH=your_fourth_token
EOF
