...
export default {
  async fetch(request, env, ctx) {
    ...
    if (TCP并发拨号数 !== 1 && 识别运营商(request) === "cmcc")
      TCP并发拨号数 = 1;
    if (env.PROXYIP) {
      ...
      反代IP =
        `${request.cf.colo}.${特征码字典[0]}.${特征码字典[1]}SsSs.nEt`.toLowerCase();
    const 访问IP =
    ...
    if (访问路径 === "version") {
      ...
      if (uuidRegex.test(请求UUID)) {
        ...
        if (
          请求前8总和 === 目标前8总和 &&
          请求UUID.slice(-12) === 目标UUID.slice(-12)
        )
          return new Response(
            JSON.stringify({
          ...
          const 订阅TOKEN = await MD5MD5(host + userID),
            作为优选订阅生成器 =
              ...
              UA.toLowerCase().includes(
                "tunnel (https://github.com/" + 特征码字典[1] + "/edge",
              );
  ...
  async function 构建预加载竞速候选列表(address, port) {
    ...
    const ipv4List = [
      ...new Set(
        aRecords.flatMap((r) => {
          const data = r.data;
          return r.type === 1 && typeof data === "string" && isIPv4(data)
            ? [data]
            : [];
        }),
      ),
    ...
    if (ipList.length === 0) {
      log(
        `[TCP直连] ${address} 的 A/AAAA 未获得可用解析结果，预加载竞速不可用，回退到原始 hostname 直连。`,
      );
      return null;
    }
    const 选中IP列表 = ipList;
    log(
      `[TCP直连] ${address} A记录:${ipv4List.length} AAAA记录:${ipv6List.length}，使用${使用记录类型}记录，竞速拨号 ${选中IP列表.length}/${拨号上限}: ${选中IP列表.join(", ")}`,
    );
    return 选中IP列表.map((hostname, attempt) => ({
      hostname,
      port,
      attempt,
  ...
  async function connectDirect(address, port, data = null, 启用预加载 = false) {
    ...
    try {
      ...
      if (预加载候选列表) {
        ...
        log(
          `[TCP直连] 预加载竞速结果: ${winner.hostname}:${winner.port} 胜出，源域名: ${winner.resolvedFrom || address}`,
        );
      }
      ...
      if (预加载候选列表)
        log(`[TCP直连] 预加载竞速失败: ${err.message || err}`);
      throw err;
    }
  }
...
async function 读取config_JSON(
  ...
  if (!config_JSON.订阅转换配置.SUBLIST)
    config_JSON.订阅转换配置.SUBLIST = false;
...
async function 生成随机IP(request, count = 16, 指定端口 = -1) {
  ...
  const 查询参数运营商 = String(
    url.searchParams.get("cnIspCode") || "",
  ).toLowerCase();
  ...
  const cidr_url =
    运营商文件标识 === "cf"
      ...
      : `https://raw.githubusercontent.com/${特征码字典[1]}/${特征码字典[1]}/main/CF-CIDR/${运营商文件标识}.txt`;
